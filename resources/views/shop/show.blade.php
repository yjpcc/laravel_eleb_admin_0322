...
@section('content')
    ...
    <div class="jumbotron row">
        <div class="col-xs-4">
            ...
            <p>
                @if($shop->status==0)
                <a href="{{ route('check',[$shop,'check'=>1]) }}" class="btn  btn-success">审核通过</a>
                <a href="{{ route('check',[$shop,'check'=>0]) }}" class="btn  btn-danger">审核不通过</a>
                @elseif($shop->status==-1)
                <a href="{{ route('check',[$shop,'check'=>2]) }}" class="btn btn-block {{ $shop->status?'btn-success':'btn-danger' }}">启用</a>
                @else
                 <a href="{{ route('check',[$shop,'check'=>-1]) }}" class="btn btn-block btn-danger">禁用</a>
                @endif
